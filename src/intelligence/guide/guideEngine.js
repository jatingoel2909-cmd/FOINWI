import { normalizeSearchQuery, tokenizeQuery } from "../search/searchHelpers.js";
...
const PHRASE_ALIASES = Object.freeze([
  ...
  [/\bus\s+dollars?\b/gu, "usd"],
  [/\bindian\s+rupees?\b/gu, "inr"],
]);
...
export const GUIDE_FAMILIES = Object.freeze([
  ...
  },
  {
    id: "planning",
    label: "Planning",
    signals: ["inflation", "purchasing power", "rising prices"],
    clarifications: ["explain-inflation", "goal-planning", "retirement-start"],
  },
  {
    id: "deposits",
    label: "Deposits",
  ...
  },
  {
    ...
    signals: ["help", "confused", "money help", "currency", "forex", "exchange rate"],
    clarifications: ["discovery-start", "discovery-calculator", "discovery-learning", "exchange-rates"],
  },
]);
...
export function matchGuideQuery(query) {
  ...
  const hasConflictingFamily = !winner?.hasExactPhraseMatch
    && familyResult.family
    && familyId !== "discovery"
    && !["comparison", "discovery"].includes(winner?.intent?.topic)
    && winner?.intent?.topic !== familyId;
  ...
  if (winner && !hasConflictingFamily && winner.score >= HIGH_MATCH_THRESHOLD && (!runnerUp || winner.score - runnerUp.score >= AMBIGUITY_MARGIN)) {
    return {
      ...
      family: familyId ?? winner.intent.topic,
